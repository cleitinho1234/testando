...
window.addEventListener("load", async () => {
  ...
  // Atualiza os dados dos contatos (incluindo o status online)
  atualizarContatos().then(renderContacts);

  // Inicia o sinal de presença (Batimento cardíaco)
  sinalizarPresenca();
  setInterval(sinalizarPresenca, 10000); // Avisa que está online a cada 10s

  // Loop de mensagens e atualização de status visual
  setInterval(() => {
    loadMessages();
    atualizarContatos().then(renderContacts);
  }, 1500);
...
// NOVO: Função para avisar ao servidor que o usuário está online
async function sinalizarPresenca() {
  if (currentUser && currentUser.id) {
    try {
      await fetch("/updatePresence", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function atualizarContatos() {
  for (let i = 0; i < contacts.length; i++) {
    ...
    if (!user.error && user.username) {
      // Atualizamos o contato com os dados novos (incluindo o lastSeen vindo do servidor)
      contacts[i] = user;
    }
  }
  localStorage.setItem("contacts", JSON.stringify(contacts));
...
function renderContacts() {
  ...
  for (let user of contacts) {
    ...
    // Lógica Online: Se a última vez visto foi há menos de 30 segundos
    ...
    const statusLabel = isOnline ? 
      ...
      '<span style="color:gray; font-size:11px;">offline</span>';
      ...
      <div class="contact" data-id="${user.id}" style="display:flex;align-items:center; padding: 10px; cursor:pointer;">
        <div style="position:relative;">
          <img src="${user.photo || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'}"
               style="width:40px;height:40px;border-radius:50%;margin-right:10px; border: 2px solid ${isOnline ? '#25D366' : '#ccc'};">
        </div>
